...
`v0.9.2` is a **preview** release (`STABILITY_LEVEL = "preview"`). SLA: none. Production use: at your own risk. See [`docs/VERSIONING.md`](docs/VERSIONING.md) and the root [README §Alpha limitations](../README.md#alpha-limitations-read-before-adopting).
...
- **Install / integrity.** `aegis-trust@0.9.2` is **live on npm** on the `latest` dist-tag, so bare `npm install aegis-trust` resolves to `0.9.2` (or pin `@0.9.2`). Released via Block C Trusted Publisher OIDC automation (token-free, GitHub-hosted, npm ≥11.5.1) in [`.github/workflows/release-attestation.yml`](../.github/workflows/release-attestation.yml). `--provenance` is currently omitted; re-enabling it is a tracked follow-up now that the source repo is public. Customer-side integrity is verifiable today via `cosign verify-blob` against the GitHub Release `v0.9.2` attached `.tgz`. The deprecated `0.9.0-rc3` (release-integrity incident F-054) is version-scoped and no longer the default.
