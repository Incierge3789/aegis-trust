...
`aegis-trust` is a pre-GA Alpha preview maintained by a single maintainer on a
best-effort basis. There is **no SLA** and no paid support tier. The channels
below are where to go for each kind of question.
...
This is preview-stage software. Issues are triaged best-effort; there is no
guaranteed response time. For the security-report timeline specifically, see
[`SECURITY.md`](SECURITY.md) (48h acknowledgment target).
