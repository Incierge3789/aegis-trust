...
`v0.9.0` is a **preview** release (`STABILITY_LEVEL = "preview"`). SLA: none. Production use: at your own risk. See [`docs/VERSIONING.md`](docs/VERSIONING.md) and the root [README §Alpha limitations](../README.md#alpha-limitations-read-before-adopting).
...
- **FULL-mode audit-ingest is fail-closed (aligned with Python as of 0.9.0).** In `FULL` mode, a *post-authorization* audit-ingest failure now fails **closed**: the filtered result is released to the caller only after the audit record is durably accepted, and on ingest failure the call returns a type-shaped empty (AO-003 audit completeness). rc7 and earlier returned the result anyway (fail-open on audit); that Node-only gap was reconciled in 0.9.0 (see CHANGELOG). The data path itself remains fail-closed in both SDKs.
...
- **Install / integrity.** `aegis-trust@0.9.0` is **live on npm**. Use `npm install aegis-trust@rc` (resolves to 0.9.0) or pin `@0.9.0`. Released via Block C Trusted Publisher OIDC automation (token-free, GitHub-hosted, npm ≥11.5.1) in [`.github/workflows/release-attestation.yml`](../.github/workflows/release-attestation.yml). `--provenance` is intentionally omitted while the source repo is private (npm 422-rejects provenance from private repos); customer-side integrity is verifiable via `cosign verify-blob` against the GitHub Release `v0.9.0` attached `.tgz`. `dist-tags.latest` was promoted to `0.9.0` on 2026-05-30, so bare `npm install aegis-trust` resolves to 0.9.0 (the deprecated `0.9.0-rc3` / F-054 is version-scoped and no longer the default).
