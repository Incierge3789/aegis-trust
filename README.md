...
> **Publish status (rc6)**: PyPI `aegis-trust==0.9.0rc6` is **live** (2026-05-24, [registry link](https://pypi.org/project/aegis-trust/0.9.0rc6/), Published Artifact Parity Gate 5-stage all PASS). npm `aegis-trust@0.9.0-rc6` publish is **pending** (internal-ops/sprint_011 T-011-10 npm side, runner token refresh required); until it lands, `npm install aegis-trust@rc` returns rc5. The full rc6 SBOM + cosign-signed `.whl` / `.tar.gz` / `.tgz` are also attached to the GitHub Release at `v0.9.0-rc6` for byte-identical provenance verification.
...
- **SBOM / SLSA / supply-chain attestation**: not yet shipped for rc6. Tracked as `supply_chain_attestation` (P1) in internal review; planned for the CI/release infrastructure sprint.
