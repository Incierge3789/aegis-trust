...
- `src/a2a/` — reports Aegis boundary decisions as an **A2A protocol
...
- `mapDecisionToA2A` / `validateOutcomeReason`: fail-closed mapping over the
  **7 legal (outcome, reason_code) pairs** the decision engine can actually
  ...
  call.
- `negotiateExtensions` / `buildAgentCardExtension` /
  `placeDecisionMetadata`: header-negotiated activation (inactive by
  ...
  per-field against the producer's own provenance declaration (the optional
  4th parameter — `withheld_fields: ["alice"]` dies before the wire); the
  COMPLETE merged carrier is rescanned so a sibling assertion in the
  caller's metadata is refused too; and every string value is held to the
  honesty guard. This SDK cannot produce the self-report its consumer side
  rejects.
- `reduceTaskState` / `buildAuthorizationRequest`: deterministic task-state
...
- `validateDecisionSubstate`: provenance-aware per-field validation —
...
- `deriveVerificationStatus` / `assertNoProducerTrustAssertions`:
...
- `bindActivation` / `filterDecisionMetadataForDelivery`: activation bound
