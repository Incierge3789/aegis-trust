...
> **Publish status (rc6)**: PyPI `aegis-trust==0.9.0rc6` is **live** (2026-05-24, [registry link](https://pypi.org/project/aegis-trust/0.9.0rc6/), Published Artifact Parity Gate 5-stage all PASS — byte-identical local-vs-registry hashes + clean install verified). npm `aegis-trust@0.9.0-rc6` publish is **pending**: Block C OIDC automation in `.github/workflows/release-attestation.yml` will publish it on the next tag push (no token, no OTP — Trusted Publisher OIDC, requires npm ≥11.5.1). Until that lands, `npm install aegis-trust@rc` resolves to `0.9.0-rc5`. The full rc6 SBOM + cosign-signed `.whl` / `.tar.gz` / `.tgz` are attached to the GitHub Release at `v0.9.0-rc6` for byte-identical provenance verification (`cosign verify-blob`).
...
- **TypeScript**: npm currently has `aegis-trust@0.9.0-rc5` as the latest *published* preview. **rc6 npm publish is pending** (Block C OIDC automation will publish it on the next tag push; the `aegis-trust-0.9.0-rc6.tgz` artifact is meanwhile attached to the GitHub Release at `v0.9.0-rc6` for byte-identical install). Install with `npm install aegis-trust@rc` (resolves to rc5 today, rc6 once Block C runs) or pin `@0.9.0-rc5` explicitly.
...
- **SBOM / SLSA / supply-chain attestation**: CycloneDX SBOMs (node + python) and Sigstore cosign-signed SDK artifacts (npm tarball, Python wheel, sdist) are attached to the GitHub Release at `v0.9.0-rc6` (`Block B Phase 2` in `release-attestation.yml`). npm provenance attestation (`npm publish --provenance` via Trusted Publisher OIDC) lands with the next tag push (`Block C`). PyPI Trusted Publisher attestation is the remaining follow-up. Tracked as `supply_chain_attestation` in operational-trust-review.
