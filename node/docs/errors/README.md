...
| `aegis.aiNative.decisionShape` | `AegisValidationError` | `parseAuthorityDecision(decision)` / `parse_authority_decision(decision)` was handed an AI-native `decision` object that does not have the frozen wire shape: a required member missing or mistyped (`outcome`, `ledgered`, `decision_id`, `receipt_event_id`, `reason_code`, `reason_label`, `verb`, `boundary`, `allowed_fields`, `withheld_fields`, `fragment_tags`, `parts`), a tag outside the value-free label shape, a `ledgered` decision with blank ids, an unledgered decision that is not the hard-fault form (BLOCKED, blank ids, no composed tags, no `allowed_fields`, not `replayed`; the trace `parts` is preserved verbatim), or a `policy_generation` that is not a non-negative safe integer. Both SDKs raise the same code with the same message fragment (shared corpus `conformance/authority_decision_view.v0.json`). | Pass the `decision` MEMBER of a `toolCall` / `streamOpen` body (not the whole body) and check the boundary version against the AI-native v1 contract (additive-only). |
...
- v0.9.0 is a **preview** release (`STABILITY_LEVEL = "preview"`). Codes may
