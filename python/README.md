...
- **`check_with_core()` (Doctor v1, Core-backed) requires a running aegis-core gateway.** The optional Core-backed path (`check_with_core()` → `POST /api/v1/check-boundary`) adds the authoritative `REQUIRE_CHECK` outcome, JWT-subject principal binding, and a tamper-evident audit chain. It needs a deployed gateway reachable at your `base_url`; with no gateway it **fails closed** (returns `BLOCK` with reason `CORE_UNAVAILABLE`), discloses nothing, and is safe but non-functional. **If you do not run a gateway, use the local `check()` (Doctor v0) — it is fully functional standalone.** Identity for `check_with_core` is the gateway's authenticated JWT subject; pass an `agent_id` only via a `TrustContext` and only if it matches that subject (a mismatching `agent_id` is rejected, fail-closed).
